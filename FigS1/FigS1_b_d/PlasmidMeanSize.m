...
ttt=Plasmids(Phis<2)./PlasmidNum(Phis<2);
...
plot(x,y,'.','markersize',20,'color',CO(3,:));hold on;
...
a1=fill([xf,fliplr(xf)],InBetween1,CO(3,:));hold on;
a2=fill([xf,fliplr(xf)],InBetween2,CO(3,:));hold on;
set(a1,'Facealpha',0.1);
set(a1,'Edgealpha',0);
set(a2,'Facealpha',0.1);
set(a2,'Edgealpha',0);
plot(xf,yf,'-','color',CO(3,:),'LineWidth',1);hold on;
plot(xf,pfd,'--','color',CO(3,:),'LineWidth',1);hold on;
plot(xf,pfu,'--','color',CO(3,:),'LineWidth',1);hold on;

xlabel('host chromosome size','fontsize',14);
ylabel('plasmid mean size','fontsize',14);
legend off;
set(gcf,'position',[100 100 210 200]);

saveas(gcf,'PlasmidMeanSize.fig');
saveas(gcf,'PlasmidMeanSize.pdf');
...
saveas(gcf,'PlasmidMeanSize_2.fig');
saveas(gcf,'PlasmidMeanSize_2.pdf');
...
set(gcf,'position',[100 100 210 210]);
saveas(gcf,'PlasmidMeanSize_3.fig');
saveas(gcf,'PlasmidMeanSize_3.pdf');